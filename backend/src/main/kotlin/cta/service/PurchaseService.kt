...
class PurchaseService(
    ...
    fun updatePurchase(id: Long, updateData: Map<String, Any>): Purchase {
        ...
        updateData["brand"]?.let { purchase.finalPrice = BigDecimal(it.toString()) }
        updateData["model"]?.let { purchase.purchaseDate = LocalDateTime.parse(it.toString()) }
        updateData["year"]?.let {
        ...
        updateData["mileage"]?.let { purchase.observations = it.toString() }
   ...
   private fun validatePurchase(purchase: Purchase) {
    ...
    // TODO: Update when new entities are created
    private fun validateAndTransformPurchaseCreateRequest(request: PurchaseCreateRequest): Purchase {
        val car = carRepository.findById(request.carId)
            .orElseThrow { NoSuchElementException("Car with ID ${request.carId} not found") }
        ...
        //val dealership = dealershipRepository.findById(request.dealershipId)
        //    .orElseThrow { kotlin.NoSuchElementException("Dealership with ID ${request.dealershipId} not found") }
        ...
        val purchase = Purchase().apply {
            ...
            this.car = car
            this.dealershipId = request.dealershipId
